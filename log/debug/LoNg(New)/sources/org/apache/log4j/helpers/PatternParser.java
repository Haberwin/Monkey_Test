...
public class PatternParser {
    ...
    private static class DatePatternConverter extends PatternConverter {
        ...
        public String convert(LoggingEvent event) {
            ...
            try {
                return this.f8df.format(this.date);
            } catch (Exception ex) {
    ...
    public String extractOption() {
        if (this.f9i < this.patternLength && this.pattern.charAt(this.f9i) == '{') {
            int end = this.pattern.indexOf(125, this.f9i);
            if (end > this.f9i) {
                String r = this.pattern.substring(this.f9i + 1, end);
                this.f9i = end + 1;
                return r;
            }
        }
    ...
    public PatternConverter parse() {
        this.f9i = 0;
        while (this.f9i < this.patternLength) {
            ...
            switch (this.state) {
                case 0:
                    if (this.f9i != this.patternLength) {
                        if (c != ESCAPE_CHAR) {
                            this.currentLiteral.append(c);
                            break;
                        }
                        switch (this.pattern.charAt(this.f9i)) {
                            case '%':
                                this.currentLiteral.append(c);
                                this.f9i++;
                                break;
                ...
                case 3:
                    ...
                    LogLog.error(new StringBuffer().append("Error occured in position ").append(this.f9i).append(".\n Was expecting digit, instead got char \"").append(c).append("\".").toString());
                    this.state = 0;
                    break;
                    break;
                case 4:
    ...
    public void finalizeConverter(char c) {
        ...
        switch (c) {
            ...
            case 'x':
                ...
                break;
            default:
                LogLog.error(new StringBuffer().append("Unexpected char [").append(c).append("] at position ").append(this.f9i).append(" in conversion patterrn.").toString());
