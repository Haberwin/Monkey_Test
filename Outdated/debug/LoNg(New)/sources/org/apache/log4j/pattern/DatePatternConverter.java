...
public final class DatePatternConverter extends LoggingEventPatternConverter {
    ...
    /* renamed from: df */
    private final CachedDateFormat f17df;
    ...
    private DatePatternConverter(String[] options) {
        ...
        try {
            ...
            this.f17df = new CachedDateFormat(dateFormat, maximumCacheValidity);
        }
    ...
    public void format(LoggingEvent event, StringBuffer output) {
        synchronized (this) {
            this.f17df.format(event.timeStamp, output);
        }
    }
    ...
    public void format(Date date, StringBuffer toAppendTo) {
        synchronized (this) {
            this.f17df.format(date.getTime(), toAppendTo);
        }
    }
}
