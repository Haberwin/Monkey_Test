...
class ParseLog(object):
    ...
    Frame = DataFrame(columns=['type', 'source', 'package', 'info', 'time', 'file_path'])
    Issue_Subject = ""
    Issue_Description = ""
    ...
    def check_log(self, start_time):
        for _ in self.log_path.iterdir():
            if _.is_dir() and datetime.strptime(_.name, "%Y%m%d_%H%M%S") >= datetime.strptime(start_time,
                                                                                              "%Y%m%d_%H%M%S"):
...
if __name__ == "__main__":
    ...
    try:
        print(start_time)
        parse.check_log(
            start_time)  # ":\\\\{url}\{shard}{dir}\\".format(url=smb_url, shard=smb_shard, dir=upload_path))
        is_parse_log = input("Do you want to parse .dbg files now?(Input Y/N ,Default N)")
            ...
            if upload_log == 'true':
                ...
                try:
                    for log_dir in parse.ParseDir_List:
                        parse.zip_and_upload(smb, log_dir, smb_shard, upload_path, parse.Exception_list)
                finally:
                    smb.disconnect()
    except Exception:
        traceback.print_exc()
